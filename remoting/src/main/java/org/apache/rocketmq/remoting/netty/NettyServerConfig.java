...
public class NettyServerConfig implements Cloneable {
    ...
    //Netty public 任务线程池线程个数，Netty网络设置，根据业务类型会创建不同的线程池，比如处理消息发送、消息消费、心跳检测等。
    //如果该业务类型（RequestCode)未注册线程池，则由public线程执行
    private int serverCallbackExecutorThreads = 0;
    /**
     * IO线程池线程个数，主要是NameServer、Broker端解析请求、返回相应的线程个数，这类线程主要是处理网络请求的，解析请求包
     * 然后转发到各个业务线程池完成具体业务操作，然后将结果再返回调用方
     */
    private int serverSelectorThreads = 3;
    //send oneway 消息请求并发度(Broker端参数）。
    private int serverOnewaySemaphoreValue = 256;
    //异步消息发送最大并发度（Broker参数）
