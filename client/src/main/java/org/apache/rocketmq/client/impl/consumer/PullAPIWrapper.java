...
public class PullAPIWrapper {
     ...
     * @param maxNums 本次最大消息条数，默认32条
     * @param sysFlag 拉取系统标记
     * @param commitOffset 当前MessageQueue的消费进度（内存中）
     * @param brokerSuspendMaxTimeMillis 消息拉取过程中允许Broker挂起时间，默认是15s
    ...
    public PullResult pullKernelImpl(
        ...
        /**
         * 根据brokerName、BrokerId从MQClientInstance中获取Broker地址，在整个RocketMQBroker的部署结构中，相同Broker构成主从结构
         * 其BrokerId会不一样，在每次拉取消息后，会给出一个建议，下次拉取从主节点还是从节点拉取。
         */
        ...
        if (findBrokerResult != null) {
            ...
            /**
             * 如果消息过滤模式为类过滤，则需要根据主题名称、broker地址找打注册在Broker上的FilterService地址，从FilterServer拉取消息
             * 否则从Broker上拉取消息。
             */
